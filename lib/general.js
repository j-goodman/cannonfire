...
General.prototype.act = function () {
  ...
  // FOURTH,
  // If there is a defeatable army bordering you, attack it.
  if (territories.withDefeatableArmies.length) {
    target = territories.withDefeatableArmies[Math.floor(Math.random() * territories.withDefeatableArmies.length)];
    if (Math.random() * 100 > 25) {
      console.log("Attacking.");
      openWindow("Attack");
      document.getElementById('attack-prompt').innerText = "We're under attack!";
      setTimeout(function () {
  ...
  // 1. Reinforce troops
  //  ⤷ a. If the current territory
  // 2. Assess borders
  //  ⤷ a. If there is a defeatable enemy army nearby, attack it. If not...
  //  ⤷ b. If there is an unnocupied enemy-loyal territory nearby, 75/25 to invade it. If not...
  //  ⤷ c. If there is an unnocupied loyal territory nearby, 50/50 to invade it. If not...
  //  ⤷ c. If there is an unnocupied territory nearby with no declared loyalty, invade it. If not...
  //  ⤷ d. Move to a random adjacent border. If all borders are occupied by outnumbering armies...
  //  ⤷ e. Do nothing.
  console.log('');
};
