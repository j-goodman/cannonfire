...
Army.prototype.subtractTroops = function () {
  ...
  this.artillery.count -= Math.round(this.damage.artillery / 5);
...
var createPlayerArmy = function () {
  game.armies = [];
  game.playerArmy = new Army ('Arizona', null, 7);
...
var createCloneArmies = function () {
  var newArmy;
  game.cloneArmies = [];
  newArmy = new Army ('Utah', 'Confederate States of America', 10);
  game.cloneArmies.push(newArmy);
  game.armies.push(newArmy);
  newArmy = new Army ('Louisiana', 'French Empire', 13);
  game.cloneArmies.push(newArmy);
  game.armies.push(newArmy);
};
