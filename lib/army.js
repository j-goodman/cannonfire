var Army = function (locationName, flag, strength, isPlayer) {
  ...
  if (!this.isPlayer) {
    ...
    console.log(names[this.flag][Math.floor(Math.random() * names[this.flag].length)]);
    ...
    for (k=0 ; k<3 ; k++) {
      window.setTimeout(function () {
        message = (battleCommentator.message(
          ...
          defender,
          defender.location));
          if (message && !Math.floor(Math.random() * 6)) {
...
Army.prototype.defineCommenterObj = function (commentatorNode) {
  return {
    ...
    slowClear: function () {
      ...
      }.bind({commentatorNode: commentatorNode, interval: interval}), 32000);
    },
