...
Army.prototype.subtractTroops = function (attacker, commenter) {
  ...
  if (commenter.isEmpty() && this.isPlayer) {
    commenter.say("> Our men sustained only minor wounds in the fighting.");
    commenter.lineBreak();
  }

  if ((commenter.isEmpty() || commenter.equals("> Our men sustained only minor wounds in the fighting.<br>")) && !this.isPlayer) {
    commenter.say("> Their army sustained only minor wounds in the fighting.");
    commenter.lineBreak();
  }
