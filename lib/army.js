...
Army.prototype.subtractTroops = function (attacker, commenter) {
  ...
  updateTroopsWindow();
  updateScoutDisplay();
  console.log('Populating territory list...');
  populateTerritoryList();

  clearTimeout(this.timeout);
  if (this.isPlayer) {
    this.timeout = setTimeout(function () {
      console.log('Calling slowClear.');
      this.slowClear();
    }.bind(commenter), 7000);
  }
};
...
var createCloneArmies = function () {
  ...
  newArmy = new Army ('Louisiana', 'French Empire', 4);
  game.armies.push(newArmy);
  newArmy = new Army ('Manitoba', 'Coalition of Freedmen', 17);
  game.armies.push(newArmy);
};
