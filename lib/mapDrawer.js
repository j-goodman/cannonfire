...
MapDrawer.prototype.updateScaleAndOffset = function (territory) {
  ...
  if (!this.offset.x || this.animating) {
    ...
    for (i=0 ; i<30 ; i++) {
      ...
      }.bind({range: this.range, scale: this.scale, getMinAndMax: this.getMinAndMax, drawMap: this.drawMap, a: this.a, b: this.b, i: i, ctx: this.ctx, canvas: this.canvas, playerArmy: this.playerArmy, drawTerritory: this.drawTerritory}), 1000 + 24 * i); // <-- NOTE : turn 1000 back to 400, 1 second is too long
    }
    setTimeout(function () {
...
MapDrawer.prototype.drawTerritory = function (territory, offset, scale, color) {
  ...
  for (i=0 ; i<vectorMap[territory].length ; i++) {
    ...
    this.ctx.lineWidth = color === '#fff' ? 2 : 5;
    this.ctx.stroke();
  }
  this.ctx.closePath();
