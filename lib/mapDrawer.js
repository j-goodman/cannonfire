...
MapDrawer.prototype.getScaleAndOffset = function (territory) {
  var i;
  if (!this.offset.x || this.animating) {
    ...
    for (i=0 ; i<80 ; i++) {
      setTimeout(function () {
        ...
        this.scale = ((this.canvas.height / largestGap) + this.scale * 19) / 20;
        this.offset = {
          x: (0 - this.range.min.x * this.scale + this.offset.x * 19) / 20,
          y: (0 - this.range.min.y * this.scale + this.offset.y * 19) / 20,
        };
...
MapDrawer.prototype.animateMarch = function (from, to) {
  console.log(from, to);
};
