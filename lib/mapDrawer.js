...
MapDrawer.prototype.drawTerritory = function (territory, offset, scale, color) {
  ...
  this.ctx.fillText(MAP[territory].symbol, (vectorMap[territory][0].x * scale + offset.x + (this.canvas.width - this.range.gap.x) / 2), (vectorMap[territory][0].y * scale + offset.y));

  this.ctx.beginPath();
  for (i=1 ; i<vectorMap[territory].length ; i++) {
    if (i === 1) {
  ...
  this.ctx.lineTo((vectorMap[territory][1].x * this.scale + this.offset.x + (this.canvas.width - this.range.gap.x) / 2), (vectorMap[territory][1].y * this.scale + this.offset.y));
  this.ctx.stroke();
  this.ctx.strokeStyle = '#fff';
};
