...
var updateScoutDisplay = function () {
  ...
  for (i=0 ; i<nearbyArmies.length ; i++) {
    displayString += nearbyArmies[i].flag === game.playerArmy.flag ?
      "There's an allied army in " + nearbyArmies[i].location.name.toUpperCase() + ", flying " + nearbyArmies[i].adjective().toUpperCase() + " colors." :
      "We've spotted another army encamped in " + nearbyArmies[i].location.name.toUpperCase() + ", led by Gen. " + nearbyArmies[i].general.name + ". They're flying " + nearbyArmies[i].adjective().toUpperCase() + " colors.";
    displayString += '<br>';
...
var updateSupplyDisplay = function () {
  ...
  loyaltyDescription = document.getElementById('supply-loyalty-description');
  reservesHeader = document.getElementById('supply-reserves-header');
  reservesInfantry = document.getElementById('supply-reserves-infantry');
  reservesCavalry = document.getElementById('supply-reserves-cavalry');
  reservesArtillery = document.getElementById('supply-reserves-artillery');

  loyaltyDescription.innerText = "Your faction has controlled " + game.playerArmy.location.name + " for " + game.playerArmy.location.loyalty + (game.playerArmy.location.loyalty === 1 ? " turn." : " turns.");
  reservesHeader.innerText = game.playerArmy.location.name.toUpperCase() + "'S AVAILABLE RESERVES:";
  reservesInfantry.innerText = "Infantry: " + game.playerArmy.location.reserves.infantry;
  reservesCavalry.innerText = "Cavalry: " + game.playerArmy.location.reserves.cavalry;
  reservesArtillery.innerText = "Artillery: " + game.playerArmy.location.reserves.artillery;

  if (!reservesInfantry.onclick) {
    setupSupplyControls(reservesInfantry, reservesCavalry, reservesArtillery);
  }
};

var setupSupplyControls = function (infantryReserves, cavalryReserves, artilleryReserves) {
  infantryReserves.onclick = function () {
    game.playerArmy.infantry.count += game.playerArmy.location.reserves.infantry;
    game.playerArmy.location.reserves.infantry = 0;
    updateTroopsWindow();
    updateSupplyDisplay();
  };
  cavalryReserves.onclick = function () {
    game.playerArmy.cavalry.count += game.playerArmy.location.reserves.cavalry;
    game.playerArmy.location.reserves.cavalry = 0;
    updateTroopsWindow();
    updateSupplyDisplay();
  };
  artilleryReserves.onclick = function () {
    game.playerArmy.artillery.count += game.playerArmy.location.reserves.artillery;
    game.playerArmy.location.reserves.artillery = 0;
    updateTroopsWindow();
    updateSupplyDisplay();
  };
};
...
var initializeFactionWindow = function () {
  ...
  for (i=0 ; i<choose.childNodes[3].childNodes.length ; i++) {
    ...
    choose.childNodes[3].childNodes[i].onclick = function () {
      anthem = document.getElementById('Anthem, ' + this.name);
      anthem.play();
      ...
      document.body.style.borderColor = game.playerArmy.lineColor();
      document.body.style.color = game.playerArmy.lineColor();

      game.playerArmy.location.flag = this.name;
      game.playerArmy.location.loyalTo = this.name;
...
var setupSidebar = function () {
  ...
  sidebar.highlightIcon = function (icon) {
    var flashing;
    // this[icon].style.background = '#fff';
    // this[icon].style.color = '#000';
    // clearInterval(this[icon].flashing);
    // this[icon].flashing = setInterval(function () {
    //   this.style.background = this.style.background === '#000' ? '#fff' : '#000';
  ...
  for (i=0 ; i<windows.length ; i++) {
    ...
    switch (windu.id) {
      ...
      case 'March':
        ...
        windu.style.width = perX(320);
        windu.style.height = 'auto';
        break;
      case 'Attack':
        windu.style.left = perX(346);
        windu.style.top = perY(255);
        windu.style.width = perX(760);
        windu.style.height = 'auto';
        break;
      case 'Commands':
